...
class YoLov5TRT(object):
    ...
    # Synchronize the stream
        ...
        for i in range(len(result_boxes)):
            tl = (round(0.002 * (image_raw.shape[0] + image_raw.shape[1]) / 2) + 1) 
            tf = max(tl - 1, 1)  # font thickness
            #line/font thickness
            color = (0,255,255)
            x=result_boxes[i]
            ...
            cv2.putText(image_raw,label, (c1[0], c1[1] - 2),0,tl / 3,[225, 255, 255],thickness=tf,lineType=cv2.LINE_AA,)
        #Save image
        global num
    ...
    def xywh2xyxy(self, origin_h, origin_w, x):
        ...
        """
        y = np.zeros_like(x) 
        #if isinstance(x, torch.Tensor) else np.zeros_like(x)
...
def open_cam_rtsp(cameraName):
    gst_str = ('rtspsrc location={} latency={} ! '
               'rtph264depay ! h264parse ! omxh264dec ! '
               'nvvidconv ! '
               'video/x-raw, width=(int){}, height=(int){}, '
               'format=(string)BGRx ! '
               'videoconvert ! appsink').format(cameraName, 200, 1080, 720)
...
class RTSCapture(cv2.VideoCapture):
    ...
    def create(url,*schemes):
        gst_str = ('rtspsrc location={} latency={} ! '
               ...
               'video/x-raw, width=(int){}, height=(int){}, '
               'format=(string)BGRx ! '
               'videoconvert ! appsink').format(url, 200, 1280, 720)
        rtscap = RTSCapture(gst_str,cv2.CAP_GSTREAMER)
...
if __name__ == "__main__":
    ...
    rtspCap=RTSCapture.create(camera_addr)
    rtspCap.start_read()
    # load custom plugins
    ...
    while rtspCap.isStarted:
            ret,Frame=rtspCap.read_latest_frame()
            if ret==True:
